...
func init() {
	...
	device.RegDevice["FUJITSU_IN"] = &FUJITSU{}
	device.RegDevice["FUJITSU_OUT"] = &FUJITSU{}
}
...
func (d *FUJITSU) insideStatus(ret device.Dict, iarray []int) {
	...
	for gzcode, gzv := range MALFUNCTIONDET {
		if gzcode == iarray[16] {
			ret["故障内容"] = gzv
			break
		} else {
	...
	ret["温度上下限设置状态(制冷/干燥)"] = fmt.Sprintf("下限=%0.1f,上限=%0.1f", float64(iarray[31])/4, float64(iarray[30])/4)
	ret["温度上下限设置状态(加热)"] = fmt.Sprintf("下限=%0.1f,上限=%0.1f", float64(iarray[33])/4, float64(iarray[32])/4)
	ret["温度上下限设置状态(自动)"] = fmt.Sprintf("下限=%0.1f,上限=%0.1f", float64(iarray[35])/4, float64(iarray[34])/4)
...
func (d *FUJITSU) outsideStatus(ret device.Dict, iarray []int) {
...
func (d *FUJITSU) allInStatus(ret device.Dict, iarray []int) {
	ret["所有室内机故障监控"] = ALLINMALFUNCTION[iarray[1]]
	ret["所有室内机开/关状态"] = ALLONOFF[iarray[3]]
}

func (d *FUJITSU) encode(m device.Dict) (results json.Number, err error) {
	name, _ := m["_varname"]
	if d.mtype == "outM" {
